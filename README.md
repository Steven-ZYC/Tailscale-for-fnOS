...
当前 fnOS 社区开发版为 **fnos.0.4（测试版）**。FPK 完整版本及正式
GitHub 发布标签采用 `v<Tailscale版本>-fnos.<社区版本>` 格式，例如
`v1.102.2-fnos.0.4`：
...
今后的正式发布只使用完整标签；`v0.x` 这类简短标签不再作为正式发布标签。
仓库中已有的 `v0.1` 仅作为早期源码里程碑保留。
...
- 界面分为概览、设备、设置三页，并可分别调节字体大小和界面缩放；v0.4
  将此前字体 120% 与界面缩放 80% 重新定义为新默认 100%，显示偏好只保存在
